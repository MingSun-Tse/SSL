...
Dtype SGDSolver<Dtype>::GroupLassoRegularize(int param_id) {
  ...
  case Caffe::GPU: {
    ...
    if (if_learn_kernel_shape) {
        ...
        // std::cout << "param_id:" << param_id << "   "
                  // << "group_size:" << group_size << "   "
                  // << "equivalent_ch:" << equivalent_ch << "   "
                  // << "groups:" << net_param_groups[param_id] << std::endl; // groups = 1 for cifar10
        
        for (int g=0;g<net_param_groups[param_id];g++){
    		int offset = g*group_size*equivalent_ch; // 0
			...
			Dtype term;
			caffe_gpu_asum(equivalent_ch,
                           temp_[param_id]->gpu_data()+offset,
                           &term);
			regularization_term += term*local_kernel_shape_decay;
    	}
    	caffe_gpu_div_checkzero(net_params[param_id]->count(),
                                net_params[param_id]->gpu_data(),
                                temp_[param_id]->gpu_data(),
                                temp_[param_id]->mutable_gpu_data());
