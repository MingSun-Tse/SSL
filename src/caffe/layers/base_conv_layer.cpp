...
void BaseConvolutionLayer<Dtype>::WeightAlign(){
    ...
    this->blobs_[0]->WriteToNistMMIO(layerparam.name()+".weight"); // ??WriteToNistMMIO

    ConvolutionParameter conv_param = this->layer_param_.convolution_param();
    const int M = this->blobs_[0]->shape(0)/group_; // num_row_per_group
    const int N = this->blobs_[0]->count(1,4); // num_col
    const int weight_offset = this->blobs_[0]->count()/group_; // num_weight_per_group
    ...
    switch(conv_param.conv_mode()){
        ...
        case caffe::ConvolutionParameter_ConvMode_LOWERED_CCNMM: {
            ...
            // std::cout << group_ << " " << this->blobs_[0]->count(1,4) << std::endl;
            for (int g = 0; g < group_; ++g) {
...
//                left_columns_.push_back(left_cols);
                ...
                caffe_cpu_concatenate_rows_cols(
                        conv_out_channels_ /group_, // num_row_per_group
                        kernel_dim_, // num_col
                        this->blobs_[0]->cpu_data() + weight_offset_ * g,
                ...
                weight_offset_sum += left_rows_[g]*left_columns_[g]; // 每个group留下来的参数总数
            }
